# REF: https://github.com/FoundationVision/VAR/blob/main/models/basic_var.py
...
from submodules.VAR.models.helpers import DropPath, drop_path
...
try:
    ...
    def slow_attn(query, key, value, scale: float, attn_mask=None, dropout_p=0.0):
        ...
        print("Inside slow_attn, shape of attention : ", attn.shape)
        print("Inside slow_attn, shape of attention mask : ", attn_mask.shape)
...
def slow_attn(
    ...
    # print("Shape of attn in slow_attn : ", attn_bias.shape)
    if is_causal:
    ...
    # print("ATTENTION HERE : ", attn_bias.shape)
    if enable_gqa:
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        # Replace buffer with a property
        ...
        self.using_xform = False  # We set it to false because DP doesn't support xFormers in its current state.

        # only used during inference
        self.caching, self.cached_k, self.cached_v = False, None, None

    @property
    def zero_k_bias(self):
        return torch.zeros(self._embed_dim, device=self.q_bias.device)
    ...
    def forward(self, x, attn_bias):
        ...
        # Custom KQV Implementation
        qkv_raw = self.mat_qkv(x)
        # Then add bias separately
        qkv = qkv_raw + torch.cat((self.q_bias, self.zero_k_bias, self.v_bias)).view(
            1, 1, -1
        )
        # Reshape as needed
        ...
        if using_flash:
            ...
            attn_bias = attn_bias.expand(B, self.num_heads, 680, 680)
            # print(
            #     f"Query shape: {q.shape}, Key shape: {k.shape}, Value shape: {v.shape}, Attn Bias shape: {attn_bias.shape}, Scale: {self.scale}, Batch: {B}, L:{L}, C:{C}"
            # )
            oup = (
                slow_attn(
                    query=q,
...
class AdaLNSelfAttn(nn.Module):
    ...
    def forward(self, x, cond_BD, attn_bias):  # C: embed_dim, D: cond_dim
        ...
        x = x + self.ffn(self.ln_wo_grad(x).mul(scale2.add(1)).add_(shift2)).mul(gamma2)
