# REF: https://github.com/FoundationVision/VAR/blob/main/models/var.py
...
from submodules.VAR.models.helpers import (
...
class VAR(nn.Module):
    def __init__(
        ...
        # 5. Store data for attention bias and level info computation
        # We'll use properties instead of buffers
    ...
    def lvl_1L(self):
        d = torch.cat(
            [
                torch.full((pn * pn,), i, device=self.pos_1LC.device)
                for i, pn in enumerate(self.patch_nums)
            ]
        ).view(1, self.L, 1)
        dT = d.transpose(1, 2)  # dT: 11L
        return dT[:, 0].contiguous()

    @property
    def attn_bias_for_masking(self):
        d = torch.cat(
            [
                torch.full((pn * pn,), i, device=self.pos_1LC.device)
                for i, pn in enumerate(self.patch_nums)
        ...
        dT = d.transpose(1, 2)  # dT: 11L
        attn_bias = torch.where(d >= dT, 0.0, -torch.inf).reshape(1, 1, self.L, self.L)
    ...
    def autoregressive_infer_cfg(
        ...
        # Use property instead of buffer
        lvl_1L = self.lvl_1L
        lvl_pos = self.lvl_embed(lvl_1L) + self.pos_1LC
        next_token_map = (
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            lvl_1L = self.lvl_1L
            x_BLC += (
                self.lvl_embed(lvl_1L[:, :ed].expand(B, -1)) + self.pos_1LC[:, :ed]
            )  # lvl: BLC;  pos: 1LC
