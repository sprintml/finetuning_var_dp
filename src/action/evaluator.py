# Adapted from OpenAI's FID computation toolkit: https://github.com/openai/guided-diffusion/blob/main/evaluations/evaluator.py
"""
1. Redirected the Inception model's file path and reworked its download
    logic—ditching chunked requests for a slick wget call in a new folder structure.
2. Added a shuffle to the inception score computation, blending randomness
    into the metric calculation and giving a shout-out to OpenAI's toolkit remix.
"""
...
INCEPTION_V3_PATH = "out/model_checkpoints/inception_v3/classify_image_graph_def.pb"
...
class Evaluator:
    ...
    def compute_inception_score(
        ...
        np.random.shuffle(
            activations
        )  # Github issue #153: https://github.com/openai/guided-diffusion/issues/153 & Github issue #77: https://github.com/FoundationVision/VAR/issues/77#issuecomment-2421450189
        softmax_out = []
...
def _download_inception_model():
    if not os.path.exists(INCEPTION_V3_PATH):
        ...
        os.system(f"wget -q {INCEPTION_V3_URL} -O {INCEPTION_V3_PATH}")
